...
<head>
  ...
  <link rel="stylesheet" href="{{ asset('website') }}/css/accueil.css">
...
<body>
    ...
    <div id="citation">
        "L'univers, c'est un livre, et des yeux qui le lisent."<br>
Victor Hugo, L'Océan d'en haut, VIII
    </div>
    

    <div class="row" id="photoBio">
        <div id="transitionImage"></div><img src="@if($user->image) {{ $user->image }} @else {{ asset('website/images/auteur.jpeg') }} @endif" alt="Image de l'auteur" id="authorImage">
    </div>
    <div class="row" id="biographie">
        <h3 id="titleDescription">{{ $user->name }}</h3>
        <p id="authorDescription">{{ $user->description }}</p>
    </div>
<footer>
